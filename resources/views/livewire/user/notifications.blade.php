<div class="w-full">
    ...
    <div class="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div class="flex flex-wrap gap-3 w-full lg:w-auto">
            <button wire:click="markAllAsRead" 
                class="flex-1 lg:flex-none justify-center bg-white border border-brand-200 text-gray-700 hover:bg-gray-50 px-5 py-3 rounded-xl font-bold shadow-sm transition-all duration-200 flex items-center gap-2 text-sm shrink-0">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="col-span-1 lg:col-span-5 xl:col-span-4 bg-white border border-brand-200 rounded-2xl shadow-sm flex flex-col h-[75vh] overflow-hidden">
            
            <!-- Sidebar Controls -->
            <div class="p-4 border-b border-brand-100 bg-gray-50/60 shrink-0 space-y-3">
                <div class="relative">
                    ...
                    <input type="text" wire:model.live="search" placeholder="Search in notifications..." 
                        class="w-full pl-10 pr-4 py-2 bg-white border border-brand-200 rounded-xl text-sm focus:ring-4 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all" />
                </div>
                
                <div class="flex gap-2">
                    <button wire:click="$set('filterRead', 'all')" 
                        class="flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition-all border {{ $filterRead === 'all' ? 'bg-blue-600 text-white border-blue-600 shadow-sm' : 'bg-white text-gray-600 border-brand-200 hover:bg-gray-50' }}">
                        All
                    </button>
                    <button wire:click="$set('filterRead', 'unread')" 
                        class="flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition-all border {{ $filterRead === 'unread' ? 'bg-blue-600 text-white border-blue-600 shadow-sm' : 'bg-white text-gray-600 border-brand-200 hover:bg-gray-50' }}">
                        Unread
                    </button>
                    <button wire:click="$set('filterRead', 'read')" 
                        class="flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition-all border {{ $filterRead === 'read' ? 'bg-blue-600 text-white border-blue-600 shadow-sm' : 'bg-white text-gray-600 border-brand-200 hover:bg-gray-50' }}">
                        Read
                    </button>
                </div>
            </div>
            ...
            <!-- Notifications List -->
            <div class="flex-1 overflow-y-auto divide-y divide-brand-100">
                @forelse($notifications as $notification)
                    @php
                        ...
                        $iconColorClass = match(true) {
                            ...
                            default => 'bg-gray-50 text-gray-600 border border-brand-200'
                        };
                    @endphp
                    <div 
                ...
                @empty
                    <div class="p-10 text-center text-gray-500 my-auto">
                        <div class="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-3 border border-brand-100">
            ...
            @if($notifications->hasPages())
                <div class="p-4 border-t border-brand-100 bg-gray-50/50 shrink-0">
                    {{ $notifications->links() }}
                </div>
            @endif
        </div>
        ...
        <div class="col-span-1 lg:col-span-7 xl:col-span-8 bg-white border border-brand-200 rounded-2xl shadow-sm flex flex-col h-[75vh] overflow-hidden">
            @if($selectedNotification)
                <!-- Header -->
                <div class="px-6 py-5 border-b border-brand-100 bg-gray-50/40 flex justify-between items-center shrink-0">
                    <div class="flex items-center gap-3">
                        <span class="material-symbols-outlined text-gray-400 bg-white p-2 rounded-xl border border-brand-200">info</span>
                        <div>
                ...
                <div class="p-6 md:p-8 overflow-y-auto space-y-6 flex-1">
                    ...
                    @if($relatedDetails)
                        <div class="border border-brand-200 rounded-2xl overflow-hidden shadow-sm">
                            ...
                            @if(strtolower($selectedNotification->related_model) === 'appointment')
                                <div class="px-5 py-4 bg-blue-600 text-white flex justify-between items-center">
                                    <h3 class="font-bold flex items-center gap-2 text-sm text-white mb-0">
                                    ...
                                    <span class="font-mono text-xs bg-white/20 px-3 py-1 rounded-lg font-bold border border-white/15 text-white">
                                        {{ $relatedDetails->booking_number ?? $relatedDetails->tracking_code }}
                                    </span>
                                </div>
                                <div class="p-5 md:p-6 bg-white space-y-5">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                        ...
                                        <hr class="md:col-span-2 border-brand-100 my-1">
                                        <div>
                            ...
                            @elseif(strtolower($selectedNotification->related_model) === 'message')
                                ...
                                <div class="p-5 md:p-6 bg-white space-y-5">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                        ...
                                        <div class="md:col-span-2 bg-gray-50 border border-brand-200 rounded-xl p-4 mt-2">
                    ...
                    @else
                        ...
                        @if($selectedNotification->related_model)
                            <div class="border border-dashed border-brand-200 bg-gray-50/50 rounded-2xl p-6 text-center text-gray-500">
            ...
            @else
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-8 text-center text-gray-500">
                    <div class="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mb-4 border border-brand-100">
